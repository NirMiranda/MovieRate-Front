...
const UserMovies = ({ userId }: { userId: string }) => {
  ...
  useEffect(() => {
    const fetchUserMovies = async () => {
      try {
        if (userId) {
          const response = await axios.get<movie[]>(`https://193.106.55.175/user/getMoviesByUserId/${userId}`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`https://193.106.55.175/movie/deleteMoviebyid/${movieIdToDelete}`);
